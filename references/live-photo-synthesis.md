...
服务端自动处理所有 live_photo 条目和 images 静图，**按 images[] 原始下标顺序合成**：
- 遍历 images[]，对每张图检查是否匹配某个 `live_photo[pi].image`（通过URL文件名匹配）
- 如果匹配：先播 motion[pi] 动图（ss=0），再播静图
- 如果不匹配：只播静图
- `live_photo[].image` 只是 `images[]` 的子集（如9张图但只有3段标了动图），必须全部合进去
- 每张静图展示1.5秒，重复3个周期

```
具体例子（images=9张, live_photo=3段对应images[1],[4],[6]）：
  周期1:  images[0] → motion[0] → images[1] → images[2] → images[3]
         → motion[1] → images[4] → images[5]
         → motion[2] → images[6] → images[7] → images[8]
  周期2-3: 同上（动图只播第1次，后续只播静图）
```

❌ 错误做法：`motion[0] → ALL 9 images → motion[1] → ALL 9 images → ...`（非原作品顺序）

**JSON格式（v2+）：**
```json
{
  "live_photo": [{"video":"...", "image":"..."}, ...],  // 动图段
  "images": ["url0", "url1", ..., "url8"]                // 所有静图
}
```

## 配乐处理（AI 本地 ffmpeg 合成）

抖音解析接口返回 `data.music` 字段：
...
| 条件 | 操作 |
|------|------|
| ✅ `data.music.url` 存在 | 下载为 bgm.mp3，100%音量循环做背景（`-stream_loop -1`） |
| ❌ `data.music` 不存在 | 降级取第1段视频原声（volume=1.3，`-stream_loop 6` 循环） |

> ❌ 错误做法：直接放大第1段原声到170%（效果差，背景杂乱有环境音/人声）
> ✅ 正确做法：优先用抖音原配乐，100%音量，音质干净精准
...
|| **ALL images[] 全部合入** | `live_photo[].image` 只是子集。必须从 `data.images[]` 取所有图片作静图段，每张1.5s |
|| **配乐优先** | 有 `data.music.url` → 100%原配乐做背景；无 → 取第1段原声 volume=1.3 降级 |
|| **无配乐也正常合成** | 降级到第1段视频原声循环，不可报错跳过 |
